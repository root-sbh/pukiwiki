...
//
// pcommentv plugin - Show/Insert comments into specified (another) page

// Usage: #pcommentv([page][,max][,options])
//
//   page -- An another page-name that holds comments
//           (default:PLUGIN_PCOMMENTV_PAGE)
//   max  -- Max number of recent comments to show
//           (0:Show all, default:PLUGIN_PCOMMENTV_NUM_COMMENTS)
//
// Options:
//   above -- Comments are listed above the #pcommentv (added by chronological order)
//   below -- Comments are listed below the #pcommentv (by reverse order)
//   reply -- Show radio buttons allow to specify where to reply

// Default recording page name (%s = $vars['page'] = original page name)
define('PLUGIN_PCOMMENTV_PAGE', '[[Comments/%s]]');
define('PLUGIN_PCOMMENTV_PAGE_COMPATIBLE', '[[コメント/%s]]'); // for backword compatible of 'ja' pcommentv

define('PLUGIN_PCOMMENTV_NUM_COMMENTS',     10); // Default 'latest N posts'
define('PLUGIN_PCOMMENTV_DIRECTION_DEFAULT', 1); // 1: above 0: below
define('PLUGIN_PCOMMENTV_SIZE_MSG',  70);
define('PLUGIN_PCOMMENTV_SIZE_NAME', 15);

// Auto log rotation
// define('PLUGIN_PCOMMENTV_AUTO_LOG', 0); // 0:off 1-N:number of comments per page

// Update recording page's timestamp instead of parent's page itself
// define('PLUGIN_PCOMMENTV_TIMESTAMP', 0);

// ----
// define('PLUGIN_PCOMMENTV_FORMAT_NAME',	'[[$name]]');
// define('PLUGIN_PCOMMENTV_FORMAT_MSG',	'$msg');
// define('PLUGIN_PCOMMENTV_FORMAT_NOW',	'&new{$now};');

// "\x01", "\x02", "\x03", and "\x08" are used just as markers
// define('PLUGIN_PCOMMENTV_FORMAT_STRING',
// 	"\x08" . 'MSG' . "\x08" . ' -- ' . "\x08" . 'NAME' . "\x08" . ' ' . "\x08" . 'DATE' . "\x08");

function plugin_pcommentv_action()
{
	// global $vars;

	// if (PKWK_READONLY) die_message('PKWK_READONLY prohibits editing');
    die_message('PCOMMENTV prohibits action');

	// if (! isset($vars['msg']) || $vars['msg'] == '') return array();
	// $refer = isset($vars['refer']) ? $vars['refer'] : '';

	// $retval = plugin_pcommentv_insert();
	// if ($retval['collided']) {
	// 	$vars['page'] = $refer;
	// 	return $retval;
	// }
...
function plugin_pcommentv_convert()
{
	global $vars;
	...
	if (isset($params['_args'][0]) && $params['_args'][0] != '') {
		...
		if (!is_page($raw_page)) {
			...
			$page_compat = sprintf(PLUGIN_PCOMMENTV_PAGE_COMPATIBLE, $raw_vars_page);
...
// function plugin_pcommentv_insert()
// {
...
// 	$msg = str_replace('$msg', rtrim($vars['msg']), PLUGIN_PCOMMENTV_FORMAT_MSG);
// 	$name = (! isset($vars['name']) || $vars['name'] == '') ? $_no_name : $vars['name'];
// 	$name = ($name == '') ? '' : str_replace('$name', $name, PLUGIN_PCOMMENTV_FORMAT_NAME);
// 	$date = (! isset($vars['nodate']) || $vars['nodate'] != '1') ?
// 		str_replace('$now', $now, PLUGIN_PCOMMENTV_FORMAT_NOW) : '';
// 	if ($date != '' || $name != '') {
// 		$msg = str_replace("\x08" . 'MSG'  . "\x08", $msg,  PLUGIN_PCOMMENTV_FORMAT_STRING);
...
// 		if (PLUGIN_PCOMMENTV_AUTO_LOG) {
// 			$_count = isset($vars['count']) ? $vars['count'] : '';
// 			plugin_pcommentv_auto_log($page, $dir, $_count, $postdata);
// 		}

// 		$postdata = join('', $postdata);
// 	}
// 	page_write($page, $postdata, PLUGIN_PCOMMENTV_TIMESTAMP);

// 	if (PLUGIN_PCOMMENTV_TIMESTAMP) {
...
// Auto log rotation
// function plugin_pcommentv_auto_log($page, $dir, $count, & $postdata)
// {
// 	if (! PLUGIN_PCOMMENTV_AUTO_LOG) return;

// 	$keys = array_keys(preg_grep('/(?:^-(?!-).*$)/m', $postdata));
// 	if (count($keys) < (PLUGIN_PCOMMENTV_AUTO_LOG + $count)) return;

// 	if ($dir) {
// 		// Top N comments (N = PLUGIN_PCOMMENTV_AUTO_LOG)
// 		$old = array_splice($postdata, $keys[0], $keys[PLUGIN_PCOMMENTV_AUTO_LOG] - $keys[0]);
// 	} else {
// 		// Bottom N comments
// 		$old = array_splice($postdata, $keys[count($keys) - PLUGIN_PCOMMENTV_AUTO_LOG]);
// 	}
...
// 	// Recurse :)
// 	plugin_pcommentv_auto_log($page, $dir, $count, $postdata);
// }

// Check arguments
function plugin_pcommentv_check_arg($val, & $params)
{
	if ($val != '') {
...
function plugin_pcommentv_get_comments($page, $count, $dir, $reply)
{
	global $_msg_pcommentv_restrict;

	if (! check_readable($page, false, false))
		return array(str_replace('$1', $page, $_msg_pcommentv_restrict));

	$reply = (! PKWK_READONLY && $reply); // Suprress radio-buttons

	$data = get_source($page);
	$data = preg_replace('/^#pcommentv\(?.*/i', '', $data);	// Avoid eternal recurse
